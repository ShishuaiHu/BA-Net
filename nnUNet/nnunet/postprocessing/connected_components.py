...
def determin_postprocess_main():
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--base", required=True, help="base folder")
    parser.add_argument("--gt_folder", required=True, help="gt folder")
    parser.add_argument("--raw_subfolder_name", required=True, help="raw subfolder name")
    args = parser.parse_args()

    base = args.base
    gt_labels_folder = args.gt_folder
    raw_subfolder_name = args.raw_subfolder_name

    determine_postprocessing(base, gt_labels_folder, raw_subfolder_name=raw_subfolder_name,
                             temp_folder="temp",
                             final_subf_name="validation_final", processes=default_num_threads,
                             dice_threshold=0, debug=False,
                             advanced_postprocessing=False,
...
def postprocess_main():
    ...
    parser.add_argument('-l', nargs='+', type=str, required=True, help="List of label IDs (integer values)")
    args = parser.parse_args()
    ...
    for i in label_list:
        if '[' not in i:
            ...
            tmp_i = i.replace('[', '').replace(']', '')
            tmp_i = tmp_i.split(',')
            tmp_list.append([int(j) for j in tmp_i])
    label_list = tmp_list
