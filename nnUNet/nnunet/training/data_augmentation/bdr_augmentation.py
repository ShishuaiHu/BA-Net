...
from nnunet.utilities.boundary import get_all_boundary
...
class BDRsampleTransform(AbstractTransform):
    '''
        data_dict['output_key'] will be a list of boundary scaled according to ds_scales
    '''

    def __init__(self, input_key="target", output_key="bdr", width=3):
        self.output_key = output_key
        self.input_key = input_key
        self.width = width

    def __call__(self, **data_dict):
        data_dict[self.output_key] = get_all_boundary(data_dict[self.input_key], self.width)
        return data_dict


def get_BDR_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
    ...
    tr_transforms.append(BDRsampleTransform('target', 'bdr'))
    ...
    if deep_supervision_scales is not None:
        if soft_ds:
            ...
            tr_transforms.append(DownsampleSegForDSTransform3(deep_supervision_scales, 'bdr', 'bdr', classes))
        else:
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                              output_key='target'))
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='bdr',
                                                              output_key='bdr'))

    tr_transforms.append(NumpyToTensor(['data', 'target', 'bdr'], 'float'))
    ...
    val_transforms.append(BDRsampleTransform('target', 'bdr'))
    ...
    if deep_supervision_scales is not None:
        if soft_ds:
            ...
            val_transforms.append(DownsampleSegForDSTransform3(deep_supervision_scales, 'bdr', 'bdr', classes))
        else:
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                               output_key='target'))
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='bdr',
                                                               output_key='bdr'))

    val_transforms.append(NumpyToTensor(['data', 'target', 'bdr'], 'float'))
